...
void *resizeRegion(void *r, size_t newSize) {
    ...
    if (oldSize >= newSize) {
        return r;
    } else if(!nextBlock->allocated) {   // borrow space from the next block
        spaceCheck = oldSize + nextSize + prefixSize + suffixSize + 8;
        if(spaceCheck >= newSize) {
            void* newNext = (void*)currBlock + prefixSize + suffixSize + newSize;
            void* newEnd = (void*)nextBlock->suffix;
            makeFreeBlock(newNext, newEnd - newNext);
            void* limit = (void*)currBlock + newSize;
            BlockSuffix_t* newSuffix = limit - align8(sizeof(BlockSuffix_t));
            currBlock->suffix = newSuffix;
            newSuffix->prefix = currBlock;
            currBlock->allocated = 1;   // should already be allocated but jic
